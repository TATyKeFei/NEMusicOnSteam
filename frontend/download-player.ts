...
export function downloadScript(value: number): string {
  ...
  return `(async () => {
    ...
    const id = [playingState?.resourceTrackId, track?.resourceId, track?.id]
    ...
    const name = String(track?.name || metadata?.title || '').trim();
    const artist = String(artists || metadata?.artist || '').trim();
